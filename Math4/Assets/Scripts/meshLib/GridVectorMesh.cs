...
public class GridVectorMesh : VectorMesh {
	...
	public void Refresh(){
		...
		Vector3[] _vertices = new Vector3[resolution];
		Vector2[] _uvs = new Vector2[resolution];
		int[] _lines = new int[resolution];

		float diffx = girdSizeX/2;
		float diffy = girdSizeY/2;

		for (int x = 0; x < ((column+1)*2); x+=2) {
			_vertices [x] = new Vector3 (startPosition.x + (diffx * (float)x), startPosition.y, 0);
			_vertices [x+1] = new Vector3 (startPosition.x + (diffx * (float)x), endPosition.y, 0);
		}
		for (int y = 0; y < ((row+1)*2); y+=2) {
			_vertices [(column*2)+2 + y] = new Vector3 (startPosition.x, endPosition.y - (diffy * (float)y), 0);
			_vertices [(column*2)+2 + y+1] = new Vector3 (endPosition.x, endPosition.y - (diffy * (float)y), 0);
		}
